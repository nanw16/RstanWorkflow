...
\tikz{ %
  ...
  \tikzstyle{det} = [latent, double];
    ...
    \node[det, above=of probability]                  (cumProb)
      {Cumul. Prob\\\footnotesize{$P_{nk} = \textrm{logit}^{-1} (c_k - \sum_D \beta_d x_{nd})$}};
    ...
    \node[right, xshift = 0.5cm,  text width = 3.05cm, font = \scriptsize] at (data.east) {Notes: $c_K = \infty, P_{n0}=0, c_{k} < c_{k+1}$.  In Stan, use the ordered\_logistic distribution which takes linear predictors and cutoffs as parameters.};
