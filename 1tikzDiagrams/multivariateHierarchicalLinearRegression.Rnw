...
\tikz{ %
  ...
  \tikzstyle{det} = [latent, double];
    ...
    \node[det, above=of Data]        (Mean)
      {Mean\\$\mu_{n} = \sum_K \beta_{j_n k} x_{nk}$};
    \node[latent, right=of Mean, xshift = 0.75cm, yshift = -0.5cm]       (StdDev)
      {Std.Dev\\$\sigma$};
    \node[observed, above=of Mean, xshift = 1.9cm]      (predictor)
      {Predictor\\$x_{nk}$};
    \node[det, right=of predictor, xshift = 0.5cm]      (coefficient)
      {Coefficient\\$\beta_{jk} = \beta_j[k] $};
    ...
    %   {\phantom{test}};
    \node[det, right=of groupMean, font = \scriptsize]       (covarMatrix)
     {Covariance$[K\times K]$\\$\Sigma = \textrm{diag}(\tau) \Omega \textrm{diag}(\tau)$};
    \node[latent, above=of groupCoefficient, font = \footnotesize,xshift = 0.5cm]                       (tau)
     {Prior Std Dev for $\beta_{\cdot k}$\\$\tau_k \thicksim \textrm{Cauchy}(0,2.5)$};
    \node[latent, above=of covarMatrix, font = \footnotesize, xshift = 1cm]         (Omega)
     {Correlation$[K\times K]$\\$\Omega \thicksim \textrm{LKJCorr}(2)$};
    \node[left=of groupCoefficient, xshift = 1.33cm]      (phant2)
     {\phantom{test}};
    \node[below=of coefficient, yshift = 1cm]      (phant3)
     {\phantom{test}};
    \node[above=of grpPredictor, yshift = -1cm]      (phant4)
     {\phantom{test}};
    \node[above=of Omega, yshift = 0cm, xshift = -3cm]      (phant5)
     {\phantom{test}};
     
    \plate[inner sep = 0.25cm]  {obsPlate}   {(Data) (Mean) (predictor) (group)}    {Individual $n$};
    \plate[]  {predPlate}   {(coefficient) (predictor) (groupCoefficient) (phant1) (priorMeanOnk) (tau)}             {Individual Predictor $k$};
    ...
    \edge {covarMatrix}    {coefVector};
    % \edge {tau}       {covarMatrix};
    \edge {Omega}     {covarMatrix};

    \coordinate (out1) at (phant5.west);
%    \coordinate (out2) at (predPlate.north east);
