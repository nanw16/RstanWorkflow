# Simulated Data Generation for Ordered Logistic Regression Model
# Author: Nan Wu
...
pr[,1] <- invlogit(c[1] - z)
pr[,K] <- 1- invlogit(c[K-1] - z)

if(K > 2){
  for(k in 2:(K-1)){
    pr[,k] <- invlogit(c[k] - z) - invlogit(c[k-1] - z)
  }
}
...
# Write data to dataOrderedLogisticRegression.csv
write.csv(simData, file.path(directory,"dataOrderedLogisticRegression.csv"),
          row.names = FALSE)

# Write parameter to parMultLogitRegression.csv
write.csv(knownPar, file.path(directory,"parOrderedLogisticRegression.csv"),
          row.names = FALSE)
