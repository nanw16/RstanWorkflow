# Estimation of the probability model for one predictor logit regression model
# Author: Nan Wu
...
simData <- read.csv(file.path(dataDir, "dataLogitRegressionOnePredictor.csv"),
...
fitLogitRegModel = stan(
  file = file.path(stanDir, "logitRegressionOnePredictor.stan"), data = reg_dat,
  iter = 6000, chains = 4, thin = 6)
...
write.csv(postSamples, file.path(modelDir, "postLogitRegressionOnePredictor.csv"),
          row.names = FALSE)
