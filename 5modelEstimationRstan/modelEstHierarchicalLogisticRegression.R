# Estimation of the probability model for hierarchical logistic regression model
# Author: Nan Wu
...
simData <- read.csv(file.path(dataDir, "dataHierarchicalLogisticRegression.csv"),
                    stringsAsFactors = FALSE) %>%
  tbl_df()

reg_dat <- list(N = nrow(simData), x = as.matrix(select(simData,contains("x"))), 
                ll = simData$ll, y = simData$y)

D <- ncol(reg_dat$x)
L <- length(unique(reg_dat$ll))

fitLogitRegModel = stan(
  file = file.path(stanDir, "hierarchicalLogisticRegression2.stan"), data = reg_dat,
  iter = 2000, chains = 4, control = list(adapt_delta = 0.8))
...
write.csv(postSamples, file.path(modelDir, "postHierarchicalLogisticRegression.csv"),
          row.names = FALSE)
...
ggplot(subset(postSamples, grepl("mu", postSamples$varName)), aes(x = value)) + 
  ...
  ggtitle("Estimated posterior distribution for mu") +
