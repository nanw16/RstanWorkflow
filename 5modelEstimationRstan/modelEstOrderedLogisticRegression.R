# Estimation of the probability model for ordered logistic regression model
# Author: Nan Wu
...
simData <- read.csv(file.path(dataDir, "dataOrderedLogisticRegression.csv"),
...
fitLogitRegModel = stan(
  file = file.path(stanDir, "orderedLogisticRegression.stan"), data = reg_dat,
  iter = 4000, chains = 4)
...
write.csv(postSamples, file.path(modelDir, "postOrderedLogisticRegression.csv"),
          row.names = FALSE)
