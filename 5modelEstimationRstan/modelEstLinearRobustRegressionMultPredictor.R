# Estimation of the probability model for multi-predictor linear robust regression model
# Author: Nan Wu
...
simData <- read.csv(file.path(dataDir,"dataLinearRobustRegressionMultPredictor.csv"),
...
fitLinearRegModel = stan(
  file = file.path(stanDir, "linearRobustRegressionMultPredictor.stan"), 
  data = reg_dat, iter = 2000, chains = 4)
...
write.csv(postSamples, 
          file.path(modelDir, "postLinearRobustRegressionMultPredictor.csv"),
          row.names = FALSE)
