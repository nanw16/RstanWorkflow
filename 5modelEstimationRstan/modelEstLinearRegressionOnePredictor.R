# Estimation of the probability model for one predictor linear regression model
# Author: Nan Wu
...
simData <- read.csv(file.path(dataDir, "dataLinearRegressionOnePredictor.csv"),
...
fitLinearRegModel = stan(
  file = file.path(stanDir, "linearRegressionOnePredictor.stan"), data = reg_dat,
  iter = 2000, chains = 4)
...
write.csv(postSamples, file.path(modelDir, "postLinearRegressionOnePredictor.csv"),
          row.names = FALSE)
