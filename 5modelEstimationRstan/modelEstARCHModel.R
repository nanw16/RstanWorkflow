# Estimation of the probability model for an ARCH(K) model
# Author: Nan Wu
...
simData <- read.csv(file.path(dataDir, "dataARCHModel.csv"),
                    stringsAsFactors = FALSE) %>%
  tbl_df()


reg_dat <- list(T = nrow(simData), K = simData$K[1], r = simData$r)

fitARCHModel = stan(
  file = file.path(stanDir, "ARCHModel.stan"), data = reg_dat,
...
postSamples = as.data.frame(fitARCHModel) %>%
  select(-contains("_")) %>%
...
write.csv(postSamples, file.path(modelDir, "postARCHModel.csv"),
          row.names = FALSE)
