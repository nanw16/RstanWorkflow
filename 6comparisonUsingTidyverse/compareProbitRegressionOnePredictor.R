...
# for the one predictor probit regression model
# Author: Nan Wu
...
knownPar <- read.csv(file.path(parDir, "parProbitRegressionOnePredictor.csv"),
                     stringsAsFactors = FALSE) %>%
  tbl_df()

# Read posterior samples to data frame postSamples
postSamples <- read.csv(file.path(modelDir, "postProbitRegressionOnePredictor.csv"),
                        stringsAsFactors = FALSE) %>%
  tbl_df() %>%
