...
# for the multi-predictor linear regression model
# Author: Nan Wu
...
knownPar <- read.csv(file.path(parDir, "parLinearRegressionMultPredictor.csv"),
                     stringsAsFactors = FALSE) %>%
  tbl_df()

# Read posterior samples to data frame postSamples
postSamples <- read.csv(file.path(modelDir, "postLinearRegressionMultPredictor.csv"),
                        stringsAsFactors = FALSE) %>%
  tbl_df() %>%
