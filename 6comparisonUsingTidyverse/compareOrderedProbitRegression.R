...
# for the ordered probit regression model
# Author: Nan Wu
...
knownPar <- read.csv(file.path(parDir, "parOrderedProbitRegression.csv"),
                     stringsAsFactors = FALSE) %>%
  tbl_df()

# Read posterior samples to data frame postSamples
postSamples <- read.csv(file.path(modelDir, "postOrderedProbitRegression.csv"),
                        stringsAsFactors = FALSE) %>%
  tbl_df() %>%
