...
# for the Multi-level 2PL IRT Model.
# Author: Nan Wu
...
knownPar <- read.csv(file.path(parDir, "par2PLMultiLevelIRTModel.csv"),
                     stringsAsFactors = FALSE) %>%
  tbl_df()

# Read posterior samples to data frame postSamples
postSamples <- read.csv(file.path(modelDir, "post2PLMultiLevelIRTModel.csv"),
                        stringsAsFactors = FALSE) %>%
  tbl_df() %>%
