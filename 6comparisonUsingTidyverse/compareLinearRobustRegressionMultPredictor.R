...
# for the multi-predictor linear robust regression model
# Author: Nan Wu
...
knownPar <- read.csv(file.path(parDir, "parLinearRobustRegressionMultPredictor.csv"),
                     stringsAsFactors = FALSE) %>%
  tbl_df()

# Read posterior samples to data frame postSamples
postSamples <- read.csv(file.path(modelDir, "postLinearRobustRegressionMultPredictor.csv"), stringsAsFactors = FALSE) %>%
  tbl_df() %>%
